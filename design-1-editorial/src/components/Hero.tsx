...
const pillars = [
  { title: 'Education', subtitle: 'Eight welfare institutes' },
  { title: 'Craft', subtitle: 'Handmade livelihoods' },
  { title: 'Community', subtitle: 'Since 1991, Munnar' },
]
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-end overflow-hidden">
      <motion.div className="absolute inset-0 overflow-hidden" style={{ y: imageY, scale: imageScale }}>
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-charcoal/85 via-charcoal/30 to-charcoal/10 cinematic-vignette" />
      </motion.div>

      <motion.div
        style={{ y: contentY, opacity: contentOpacity }}
        className="relative w-full max-w-[1400px] mx-auto px-6 lg:px-12 pb-16 lg:pb-24 pt-28"
      >
        <motion.div
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-20 max-w-3xl">
          {pillars.map((pillar, i) => (
            <Reveal key={pillar.title} delay={0.3 + i * 0.1}>
              <div className="bg-charcoal/40 backdrop-blur-xl border border-cream/10 p-6 hover:border-cream/25 transition-colors duration-500 rounded-lg">
                <span className="text-cream/40 text-[10px] font-bold tracking-[0.2em]">0{i + 1}</span>
