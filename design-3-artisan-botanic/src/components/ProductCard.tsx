    ...
    >
      <TiltCard intensity={10}>
        <article className="group relative">
          <div className="relative aspect-[4/5] overflow-hidden bg-linen/30 mb-5 rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.08)]">
            <img
          ...
          <p className="eyebrow text-stone text-xs mb-2">{product.institute}</p>
          ...
          {product.colors && (
            <div className="flex gap-2 mt-4">
              {product.colors.map(color => (
                <span
                  key={color}
                  className="w-4 h-4 rounded-full border border-charcoal/10 bg-linen shadow-inner"
                  title={color}
                />
              ))}
            </div>
