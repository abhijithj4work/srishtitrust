...
export default function Hero() {
  ...
  return (
    <section ref={ref} className="relative min-h-screen grid grid-rows-[1fr_auto] overflow-hidden">
      ...
      <motion.div style={{ opacity: contentOpacity }} className="relative z-10 max-w-7xl mx-auto container-pad w-full pt-32 pb-8 flex flex-col">
        <motion.div
          ...
          className="max-w-3xl"
        >
          <p className="eyebrow text-cream/50 mb-8">{heroContent.eyebrow}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl pt-8">
          {pillars.map((pillar, i) => (
            <Reveal key={pillar.title} delay={0.3 + i * 0.12}>
              <div className="bg-charcoal/35 backdrop-blur-xl border border-cream/15 p-8 md:p-10 rounded-2xl hover:border-cream/30 transition-colors duration-500">
                <span className="text-terracotta text-xs font-bold tracking-[0.2em]">0{i + 1}</span>
