...
function SectionHeader({ label, title, description }: { label: string; title: string; description?: string }) {
  return (
    <div className="mb-16 md:mb-20 max-w-2xl">
      <p className="eyebrow text-warm-gray mb-6">{label}</p>
      <h2 className="display-section font-semibold text-charcoal">{title}</h2>
      {description && <p className="body-lg text-warm-gray mt-6">{description}</p>}
    </div>
  )
}
...
export default function BentoGrid() {
  ...
  return (
    <div className="bg-cream">
      <section className="section-pad border-t border-charcoal/6">
        ...
        <div className="max-w-7xl mx-auto container-pad">
          ...
          <div className="grid md:grid-cols-12 gap-6 md:gap-8">
            <Reveal className="md:col-span-7">
              <div className="h-full min-h-[320px] p-10 md:p-14 border border-charcoal/6 bg-cream/50 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.05)] hover:shadow-[0_16px_48px_rgba(0,0,0,0.1)] transition-shadow duration-500">
                <span className="eyebrow text-terracotta text-xs">01</span>
                <h3 className="text-2xl md:text-3xl font-semibold tracking-tight mt-5 mb-5">{pillars[0].title}</h3>
                <p className="body-lg text-warm-gray max-w-lg">{pillars[0].description}</p>
              </div>
            </Reveal>
            <div className="md:col-span-5 flex flex-col gap-6 md:gap-8">
              {pillars.slice(1).map((pillar, i) => (
                <Reveal key={pillar.title} delay={(i + 1) * 0.12}>
                  <div className="p-10 md:p-14 border border-charcoal/6 bg-cream/50 rounded-2xl flex-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition-shadow duration-500">
                    <span className="eyebrow text-terracotta text-xs">0{i + 2}</span>
                    <h3 className="text-xl font-semibold tracking-tight mt-5 mb-4">{pillar.title}</h3>
                    <p className="body-lg text-warm-gray">{pillar.description}</p>
                  </div>
                </Reveal>
              ))}
            </div>
          ...
          <Reveal delay={0.24}>
            <blockquote className="mt-20 md:mt-24 max-w-3xl mx-auto text-center bg-white/60 backdrop-blur-xl rounded-3xl p-12 md:p-16 shadow-[0_8px_32px_rgba(0,0,0,0.06)]">
              ...
              <cite className="eyebrow text-warm-gray mt-8 block not-italic text-xs">
              ...
              <Reveal key={inst.slug} delay={i * 0.12}>
                <TiltCard intensity={10}>
                  <Link to={`/institutes/${inst.slug}`} className="block group">
                    <div className="aspect-[3/4] overflow-hidden mb-5 rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.08)] group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.14)] transition-all duration-500">
                    ...
                    <h3 className="text-base md:text-lg font-semibold tracking-tight group-hover:text-terracotta transition-colors">{inst.name}</h3>
                    <p className="text-warm-gray text-sm mt-3 leading-relaxed line-clamp-2">{inst.tagline}</p>
                  </Link>
                </TiltCard>
              </Reveal>
            ))}
        ...
        <div className="max-w-7xl mx-auto container-pad relative z-10">
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
            ...
            <Reveal delay={0.1} className="lg:col-span-7 lg:pl-8">
              <p className="eyebrow text-cream/40 mb-8">{artisanSpotlight.title}</p>
      ...
      <section className="section-pad bg-white">
        <div className="max-w-7xl mx-auto container-pad grid lg:grid-cols-2 gap-12 lg:gap-16">
          <Reveal>
            <motion.div whileHover={{ rotateX: 2, scale: 1.01 }} style={{ transformPerspective: 1200 }} className="p-10 md:p-14 border border-charcoal/6 rounded-2xl bg-white/60 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.06)] hover:shadow-[0_16px_48px_rgba(0,0,0,0.1)] transition-shadow duration-500 h-full">
              <p className="eyebrow text-warm-gray mb-6">{womenEmpowerment.title}</p>
          ...
          <Reveal delay={0.1}>
            <motion.div whileHover={{ rotateX: 2, scale: 1.01 }} style={{ transformPerspective: 1200 }} className="p-10 md:p-14 bg-olive text-cream rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.15)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.2)] transition-shadow duration-500 h-full">
      ...
      <section className="section-pad">
        <div className="max-w-7xl mx-auto container-pad">
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-2xl mt-16 md:mt-20 shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
      ...
      <section className="section-pad bg-sand/30 border-t border-charcoal/6">
        <div className="max-w-3xl mx-auto container-pad text-center">
          <Reveal>
            ...
            <p className="body-lg text-warm-gray mb-10">Your donations expand our programs and create more sustainable livelihoods in Munnar.</p>
