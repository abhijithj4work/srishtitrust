      ...
      <div className="relative max-w-7xl mx-auto container-pad h-24 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 z-10">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setIsOpen(true)} className="relative p-3 text-mist/50 hover:text-mist transition-colors" aria-label="Open cart">
            ...
            <AnimatePresence>
              {itemCount > 0 && (
                <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-gold text-night text-[10px] rounded-full flex items-center justify-center font-bold">
                  {itemCount}
                </motion.span>
              )}
            </AnimatePresence>
            ...
            <div className="container-pad py-6 flex flex-col gap-2">
              {navLinks.map((link, i) => (
                <motion.div key={link.path} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                  <Link to={link.path} onClick={() => setMobileOpen(false)} className={`block text-base font-medium py-4 ${location.pathname === link.path ? 'text-gold' : 'text-mist/50'}`}>
                    {link.label}
                  </Link>
                </motion.div>
              ))}
